...
ElbowPlot(objSubFilt)

#ggsave(paste0('RegNormElbowPlot.png'), height = 12, width = 14, units = 'in', dpi = 300)
...
objSubFilt <- FindNeighbors(objSubFilt, dims = 1:15)
#objSubFilt = FindClusters(objSubFilt, resolution = 0.2)
objSubFilt <- RunUMAP(objSubFilt, dims = 1:15)
...
#objSubFilt <- FindClusters(objSubFilt, algorithm = 4)

#Idents(objSubFilt)<-current_resol
plotClusters = DimPlot(objSubFilt, reduction = "umap", label = T, repel=T)

#objSubFilt[["RNA"]] <- split(objSubFilt[["RNA"]], f = objSubFilt$slide_ID_numeric)

setwd("output")

ggsave(paste0('RegNorm15PCA_Groups.png'), plot=curPcPlot, height = 12, width = 14, units = 'in', dpi = 300)


ggsave(paste0('RegNorm15PCA_Clusters.png'), plot=plotClusters, height = 12, width = 14, units = 'in', dpi = 300)
...
ggsave(paste0('RegNorm_15PCA_Control_Clusters.png'), plot=contrPlot, height = 12, width = 14, units = 'in', dpi = 300)
