...
plotUmap<-function(x, curPc){
  for (i in x){
    ...
    ggsave(paste0("Integrated_Clustering/Resols/", curPc, "/Resol_",i, ".png"), plot=p2, height = 12, width = 14, units = 'in', dpi = 300)
...
objSubFilt <- RunPCA(objSubFilt)

DefaultAssay(objSubFilt)

objSubFilt <- IntegrateLayers(object = objSubFilt, method = CCAIntegration, normalization.method = "SCT", verbose = F)
ElbowPlot(objSubFilt, ndims = 50)

setwd("output")

dir.create("Integrated_Clustering/PCA/", recursive = T)
dir.create("Integrated_Clustering/Resols/", recursive = T)

#saveRDS(objSubFilt, "SctInteg")
...
for (curPc in pcs ) {
  ...
  ggsave(paste0("Integrated_Clustering/PCA/PCs_",curPc, ".png"), plot = curPcPlot, height = 12, width = 14, units = 'in', dpi = 300)
  
  contrGr = c("1_A", "2_B")
  contr = subset(objSubFilt, Slide_Group %in% contrGr)
  contrPlot = DimPlot(contr, reduction = "umap", group.by = "Slide_Group", label = F)
  ggsave(paste0("Integrated_Clustering/PCA/PCs_",curPc, "_Controls.png"), plot=contrPlot, height = 12, width = 14, units = 'in', dpi = 300)
  
  dir.create(paste0("Integrated_Clustering/Resols/", curPc), recursive = T, showWarnings = F)
...
#objSubFilt  <- PrepSCTFindMarkers(objSubFilt )
