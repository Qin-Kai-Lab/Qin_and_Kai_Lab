...
getPeakRna = function(dat, test, targetDir) {
  ...
  try({
    ...
    p2g$Cluster = cluster
    write.csv(p2g, file = paste0(targetDir, "AllCells_peakGeneCor_", curDate, ".csv"), row.names = F)
...
getPeakRna(dat = atacFilt, test = "pearson", targetDir = "atacRna/peaksGenesCor/atacMergedMacs2_2/pearson/")