...
combined.peaks <- reduce(x = c(stress_peak, control_peak))

peakwidths <- width(combined.peaks)
combined.peaks <- combined.peaks[peakwidths  < 10000 & peakwidths > 20]

peakDist<-data.frame(peakwidths)
peakDist$Cat<-'All'

ggplot(peakDist, aes(x=Cat, y=peakwidths)) + 
  geom_violin()

boxplot(peakwidths~Cat,data=peakDist)
...
gc()
# start combining
pbmc.combined <- merge(stress, control)
...
integration.anchors <- FindIntegrationAnchors(
  object.list = list(stress, control),
...
saveRDS(integrated, file = 'atacIntegrated_macs2_stressFirst')
