...
atacAnnot = function(x, targDir) {
  ...
  for ( cluster in clusters) {
    ...
    df$Cluster <- cluster
    outName = paste0(targDir, 'StressVsControl_', cluster, '_', curDate, '_Macs2IsolCluster.csv')
...
runAll = function() {
  for (cluster in clusters) {
    try({
      ...
      targDir <- './archContrStress/atacRna/StressVsControl/Macs2_IsolClust_Def/'
      dir.create(targDir)
      atacMark(x = cellsClust, targDir = targDir)
      # get annotations
      targDir <- './archContrStress/atacRna/StressVsControl/Macs2_IsolClust_Def/Annotations/'
