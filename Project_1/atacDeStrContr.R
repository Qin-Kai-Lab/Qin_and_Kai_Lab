...
dir.create('atacIntDE/stress_first')

targDir<-'atacIntDE/stress_first/'
...
findMarkersGr<-function(dat, clust, pos, latVar, test){
  ...
  for ( i in clust){
    ...
    write.csv(allMarkEdit, paste0(targDir, 'atacInt_DE_', i, '_ContrVsStress_', curDate, '.csv'), row.names = F)
...
write.csv(groupMarkers, paste0('./allDe_atacInt_StrVsContr_stressFirst_',curDate, '.csv'), row.names = F)
