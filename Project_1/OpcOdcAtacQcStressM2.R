stressCells = read.table('../output/OPC_ODC_stressCells.txt')
stressCells = stressCells$V1
...
brainFiltRna <- subset(brain, subset = gex_barcode %in% stressCells )
...
high_nss <- quantile(brainFiltRna$nucleosome_signal, probs = 0.98, na.rm = T)
low_nss <- quantile(brainFiltRna$nucleosome_signal, probs = 0.02, na.rm = T)
...
saveRDS(brainAllFilt, file = 'OpcOdc_stress_macs2')
