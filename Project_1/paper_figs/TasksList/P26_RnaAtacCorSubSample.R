...
sumGroups = table(atacFilt$dataset)
targNumb = min(sumGroups)

getCor = function(atacFilt, group, topN, allMarkersPct, corTest, targNumb) {
  Estimate = numeric()
  Pval = numeric()
  Gene = character()
  ...
  for (curGene in genesList ) {
    try({
      ...
      curExprDf = data.frame(rna_expr, atac_expr)
      selected_rows <- curExprDf[sample(nrow(curExprDf), targNumb, replace = F),]
      curCor = cor.test(selected_rows$rna_expr, selected_rows$atac_expr,  method = corTest)
...
for (group in groups ) {
  try({
    curDf = getCor(atacFilt=atacFilt, group=group, topN="all", allMarkersPct=allMarkersPct, corTest="spearman", targNumb = targNumb)
    curDf$fdr_p = p.adjust(curDf$Pval, method = "fdr")
    #outFile = paste0(targDir, "PredictGene_RNA_spearman_", group, "_2024-01-03.csv")
...
write.csv(combDf, paste0(targDir, "PredictGene_RNA_spearman_AllClusters_SepGroups_SubSamp_2024-01-16.csv"), row.names = F)

rm(list = ls()[!ls() %in% c("getCor", "targNumb", "groups")])
...
write.csv(combDf, paste0(targDir, "PredictGene_20KB_RNA_spearman_AllClusters_SepGroups_SubSamp_2024-01-16.csv"), row.names = F)