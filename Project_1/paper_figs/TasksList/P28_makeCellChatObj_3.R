...
opc = rownames(opcOdc@meta.data)[opcOdc@meta.data$newMonocClust == "OPC"]
odc = rownames(opcOdc@meta.data)[opcOdc@meta.data$newMonocClust == "ODC" ]
interm = rownames(opcOdc@meta.data)[opcOdc@meta.data$newMonocClust == "Intermideate" ]
...
editClusterNames = function(meta) {
  ...
  for ( i in 1:nrow(meta) ) {
    if ( meta$Cell_ID[i] %in%opc  ) {
      meta$Cluster[i] = "OPC"
    } else if (meta$Cell_ID[i]%in%odc) {
      meta$Cluster[i] = "ODC"
    } else if (meta$Cell_ID[i]%in%interm) {
      meta$Cluster[i] = "Intermideate"
    } else {
...
getCellChatObj = function(data.input, i, meta) {
  ...
  fileName = paste0("CellChat_RNA_", i, "_3.RDS")
