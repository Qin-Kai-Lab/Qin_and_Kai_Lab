...
# Weights
maxOdc = max(c(max(control@net$weight), max(stress@net$weight)))
maxOpc = max(c(max(control@net$weight), max(stress@net$weight)))
...
#Counts
maxOdc = max(c(max(control@net$count), max(stress@net$count)))
maxOpc = max(c(max(control@net$count), max(stress@net$count)))
...
png(paste0(targDir, "Communication_probabilites_Input.png"), width = 18, height = 24, units = "in", res = 300)
netVisual_bubble(cellchat, sources.use = c(1:6,9), targets.use = c(7,8),  comparison = c(1, 2), angle.x = 45, font.size = 16)
dev.off()

gg1 <- netVisual_bubble(cellchat, sources.use = c(1:6,9), targets.use = c(7,8),  comparison = c(1, 2), max.dataset = 2, title.name = "Increased signaling in Stress", angle.x = 45, remove.isolate = T, font.size = 16)
#> Comparing communications on a merged object
gg2 <- netVisual_bubble(cellchat, sources.use = c(1:6,9), targets.use = c(7,8),  comparison = c(1, 2), max.dataset = 1, title.name = "Decreased signaling in Stress", angle.x = 45, remove.isolate = T, font.size = 16)
#> Comparing communications on a merged object
gg3 = gg1 + gg2

png(paste0(targDir, "Communication_probabilites_StrContr_Input.png"), width = 28, height = 22, units = "in", res = 300)
print(gg3)
dev.off()


targDir = "Paper_figs/TasksList/P28/Merged_2/OPC_ODC/Pathwayas_Input/"
dir.create(targDir, recursive = T, showWarnings = F)
for ( curPathway in combPaths ) {
  ...
  for (i in 1:length(object.list)) {
    netVisual_aggregate(object.list[[i]], signaling = pathways.show, layout = "circle", edge.weight.max = weight.max[1], edge.width.max = 10, 
                        signaling.name = paste(pathways.show, names(object.list)[i]), sources.use = c(1:6,9), targets.use = c(7,8))
  }
  dev.off()
}
