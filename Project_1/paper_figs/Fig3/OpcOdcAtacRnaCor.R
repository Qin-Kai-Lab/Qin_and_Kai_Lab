...
compFrags = function(curName, extUp, extDown, subObj) {
  ...
  contrCells = subObj@meta.data$Merged_CellName[subObj@meta.data$dataset == "Control"]
  stressCells = subObj@meta.data$Merged_CellName[subObj@meta.data$dataset == "Stress"]
  filtContr1 = curChrDfContr[curChrDfContr$Merged_CellName%in%contrCells,]
  filtStress1 = curChrDfStress[curChrDfStress$Merged_CellName%in%stressCells,]
  
  
  # sum fragments per cell
  contrSum = aggregate(V5 ~ Merged_CellName, data = filtContr1, FUN = sum)
  stressSum = aggregate(V5 ~ Merged_CellName, data = filtStress1, FUN = sum)
  
  # add 0s
  addMissingCells = function(df, cellList) {
    missCells = character()
    for ( curCell in cellList ) {
      if ( !curCell%in%df$Merged_CellName ) {
        missCells = c(missCells, curCell)
      }
    }
    missDf = data.frame(Merged_CellName = missCells, V5 = 0)
  ...
  curTest = wilcox.test(combContr$V5, combStress$V5)
  
  contrRes = combContr$V5
  names(contrRes) = combContr$Merged_CellName
  
  stressRes = combStress$V5
  names(stressRes) = combStress$Merged_CellName
  
  allComb = c(contrRes, stressRes)
  
  return(allComb)
}

atac_expr = compFrags(curName="Gpr17", extUp=2000, extDown=500, subObj=atacFiltSub)

atac_expr= atac_expr[sort(names(atac_expr))]

norm_expr = atacFiltSub@assays$RNA@data["Gpr17" ,]
norm_expr = norm_expr[sort(names(norm_expr))]

gene1_expr = atacFiltSub@assays$RNA@counts["Gpr17" ,]
gene1_expr = gene1_expr[sort(names(gene1_expr))]

identical(names(atac_expr), names(norm_expr))
identical(names(atac_expr), names(gene1_expr))

spTest = cor.test(gene1_expr, atac_expr, method = "spearman")
prTest = cor.test(norm_expr, atac_expr, method = "pearson")

curDf = data.frame(Cell_ID = names(norm_expr), RNA_EXPR=norm_expr, ATAC_COUNT = atac_expr)
