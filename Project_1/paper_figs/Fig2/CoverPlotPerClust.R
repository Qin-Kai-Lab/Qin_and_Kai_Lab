...
targDir = './Paper_figs/Fig2/CoverPlots/PerClustPval/'
...
groupMarkers$Annotations = groupMarkers$Cell_Type
...
covPlotClust = function(obj, markDf, cluster, topNumb) {
  objFilt = subset(obj, subset = Annotations == cluster)
  markDfFilt = markDf[(markDf$Annotations == cluster),]
  markDfFilt$AbsLog = abs(markDfFilt$avg_log2FC)
  dfOrd = markDfFilt[order(markDfFilt$p_val_adj, decreasing = F),]
  topMark = head(dfOrd$Genes, topNumb)
  
  runCovPlot(obj = objFilt, genes = topMark, datType = cluster)
}

allClust = unique(atacFilt$Annotations)
for (curClust in allClust) {
  covPlotClust(obj = atacFilt, markDf = groupMarkers, cluster = curClust, topNumb = 5)
}